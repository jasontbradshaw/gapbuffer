...
class TestGapBuffer(unittest.TestCase):
    ...
    def test_sort_no_args(self):
        """Does sort work without specified arguments?"""
    ...
    def test_sort_cmp(self):
        """Does sort work with a comparator function specified?"""

        content = [4, 2, 1, 3, 5, 0]
        b = gapbuffer("i", content)

        c = lambda a, b: (1 if (a + 1) > (b - 1) else
                -1 if (a + 1) == (b - 1) else -1)
    ...
    def test_sort_key(self):
        """Does sort work with a key function specified?"""
    ...
    def test_sort_reverse(self):
        """Does sort work with reverse specified?"""
    ...
    def test_sort_all_args(self):
        """Does sort work with all arguments specified?"""
        ...
        b.sort(cmp=c, key=k, reverse=True)
        content.sort(cmp=c, key=k, reverse=True)

        self.assertEqual(b, content)

    def test_sort_congruency(self):
        """Does sorting yield the same sequence as the sorted iterator?"""

        b1 = gapbuffer("i", [4, 2, 1, 3, 5, 0])
        b2 = gapbuffer("i", b1)
